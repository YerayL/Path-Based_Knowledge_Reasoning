...
parser.add_argument('-g', '--get_only_relation', type=bool, default=False,
                    help="This option to be used when data has entities but we only want to extract relations")
parser.add_argument('-m', '--max_path_length', type=int, default='8', )
parser.add_argument('-t', '--max_num_types', type=int, default='7', )

args = parser.parse_args()
input_data = args.input_data
out_data = args.output_data
isOnlyRelation = args.only_relation
getOnlyRelation = args.get_only_relation
# isEc2_instance = (args.ec2_instance == '1')
MAX_POSSIBLE_LENGTH_PATH = args.max_path_length
NUM_ENTITY_TYPES_SLOTS = args.max_num_types

# gunzip the files in the vocab directory and plug it in; alternatively you can create your own vocab too.
entity_type_vocab_file = './ChainsofReasoning/vocab/entity_type_vocab.txt.gz'
relation_vocab_file = './ChainsofReasoning/vocab/relation_vocab.txt.gz'
entity_vocab_file = './ChainsofReasoning/vocab/entity_vocab.txt.gz'
entity_type_map_file = './ChainsofReasoning/vocab/entity_to_list_type.json.gz'
label_vocab_file = './ChainsofReasoning/vocab/domain-label.gz'
...
max_length = -1
train_files = ['/positive_matrix.tsv.translated',
               '/negative_matrix.tsv.translated',
               '/dev_matrix.tsv.translated',
               '/test_matrix.tsv.translated']  ##dont change the ordering or remove entries. This is bad coding, I know.

# data_dir = './data/chains_of_reasoning_eacl_dataset'

input_dir_list = os.listdir(input_data)

# input_dir = input_dir + '/' + input_dir_list[0]
